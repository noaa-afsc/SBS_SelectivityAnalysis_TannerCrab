prdMod<-function(mdl,trms,lst,type="response",keep=NULL,p=0.05){
  ...
  return(prd);
   ...
   mdl5m = mgcv::gam(emp_sel ~ ti(z,bs="tp",k=10) + ti(z,y,bs="re"),
                     family=tw,data=dfrESsp,method="REML",
                     weights=n_BSFRF/mean(n_BSFRF));
  wtsMGCV::gam.check.plots(mdl5m)
  #--on response scale
  prd5mb = prdMod(mdl5m,trms=c("(Intercept)","ti(z)"),lst=grids,type="response",keep="z");
  ggplot(prd5mb,aes(x=z,y=emp_sel,ymin=lci,ymax=uci)) + geom_line() + geom_ribbon(alpha=0.3);
  prd5m = prdMod(mdl5m,trms=NULL,lst=grids,type="response");
  ggplot(prd5m,aes(x=z,y=emp_sel,ymin=lci,ymax=uci,colour=y,fill=y)) + 
    ...
    geom_line(data=prd5mb,mapping=aes(x=z,y=emp_sel),inherit.aes=FALSE) + 
    geom_ribbon(data=prd5mb,mapping=aes(x=z,y=emp_sel,ymin=lci,ymax=uci),inherit.aes=FALSE,alpha=0.3); 
  #--on link scale
  prd5mb = prdMod(mdl5m,trms=c("(Intercept)","ti(z)"),lst=grids,type="link",keep="z");
  prd5m = prdMod(mdl5m,trms=NULL,lst=grids,type="link");
  ggplot(prd5m,aes(x=z,y=emp_sel,ymin=lci,ymax=uci,colour=y,fill=y)) + 
    geom_line() + geom_ribbon(colour=NA,alpha=0.3) + 
  ...
  dfr = gratia:::smooth_estimates(mdl5m,select="ti(z,y)",unconditional=TRUE,overall_uncertainty=TRUE) |> 
          gratia::add_confint();
  dfrPRs = dplyr::bind_cols(dfrESsp |> dplyr::filter(!is.na(emp_sel)),
                            gratia::partial_residuals(mdl5m,select="ti(z,y)") |> dplyr::rename(residuals="ti(z,y)"));
