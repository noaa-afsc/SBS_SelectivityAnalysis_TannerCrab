...
if (FALSE){  
  ...
  dfrPrd = prdMod(best_mdl,trms=c("all"),type="link",lst=dfrPrdGrd,p=0.10) |> 
              dplyr::mutate(emp_sel=exp(emp_sel),
                            lci=exp(lci),
                            uci=exp(uci));
  wtsUtilities::saveObj(dfrPrd,file.path(dirThs,"rda_Step4a.Females.BinomialModel_Best.AllPredicted.SelFcns.RData"));
}

#--calculate unweighted averages across hauls by year
dfrPrd = wtsUtilities::getObj(file.path(dirThs,"rda_Step4a.Females.BinomialModel_Best.AllPredicted.SelFcns.RData"));
dfrPrdAvg = dfrPrd |> 
              dplyr::group_by(y,z) |> 
