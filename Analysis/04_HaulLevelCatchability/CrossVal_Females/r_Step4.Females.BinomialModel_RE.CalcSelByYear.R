...
if (FALSE){  
  ...
  #--get best Binomial model
  best_mdl = wtsUtilities::getObj(file.path(dirThs,"rda_Step3b1.BinomialModels_RE.RData"));
  ...
  dfrPrd = prdMod(best_mdl,trms=c("all"),type="link",lst=dfrPrdGrd,p=0.10) |> 
              dplyr::mutate(emp_sel=exp(emp_sel),
                            lci=exp(lci),
                            uci=exp(uci));
  wtsUtilities::saveObj(dfrPrd,file.path(dirThs,"rda_Step4a.Females.BinomialModel_RE.AllPredicted.SelFcns.RData"));
}

#--calculate unweighted averages across hauls by year
dfrPrd = wtsUtilities::getObj(file.path(dirThs,"rda_Step4a.Females.BinomialModel_RE.AllPredicted.SelFcns.RData"));
dfrPrdAvg = dfrPrd |> 
              dplyr::group_by(y,z) |> 
