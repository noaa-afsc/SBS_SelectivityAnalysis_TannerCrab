...
evalAllModels<-function(mdl,
  ...
  lstAll<-foreach (i = icmbs,
    ...
    tblTst   = NULL;
    if (!is.null(dfrTest)) {
      prd = unname(predict(mdlres,dfrTest,type="link"));
      obs = dfrTest[[link]];
      tblTst = tibble::tibble(y=dfrTest$y,h=dfrTest$h,
                              d=dfrTest$d,t=dfrTest$t,f=dfrTest$f,s=dfrTest$s,
                              z=dfrTest$z,x=dfrTest$x,
                              n=dfrTest$n,p=dfrTest$p,asr=dfrTest$asr,q=dfrTest$q,
                              obsLnR=obs,prdLnR=prd);
      #--R-sqr prediction error
      use = is.finite(prd)&is.finite(obs);
      prd = prd[use]; obs = obs[use];
    ...
    tbl = tibble::tibble(i=i,
                         ...
                         test=list(tblTst=tblTst));
    if (log) cat(paste(paste0("\nResults for model ",i,": ",paste(str[2],"~",str[3])),rsqr,aic,rsqr_prd,signifp,signifs,concrv_tst,"\n",collapse=", "));
    if (debug) cat("#################################\n\n");
    if (log||debug) sink();
    tbl
  }#--foreach(i = 1:ncmbs)
