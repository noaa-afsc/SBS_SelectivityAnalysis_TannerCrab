...
#--run cross validation-------------------------------------------------
  ...
  dfrCrsVal = runCrossValidation(
                 ...
                 log=TRUE,
                 debug=FALSE);
  wtsUtilities::saveObj(dfrCrsVal,file.path(dirThs,"rda_Step3b.BinomialModels_CrsVal.Females.RData"));
}
if (FALSE){
#--evaluate best model-------------------------------------------------
  source(file.path(dirThs,"../r_gam.prefit.functions.R"));
  ...
  mdl = mdlB_ZE2D;
  if (!exists("dfrCrsVal")) dfrCrsVal = wtsUtilities::getObj(file.path(dirThs,"rda_BinomialModels_CrsVal.Females.RData"));
  ...
  wtsUtilities::saveObj(dfrCrsValdp1,file.path(dirThs,"rda_Step3b.BinomialModels_OrderedModels.Females.RData"));
  wtsUtilities::saveObj(best_mdl,    file.path(dirThs,"rda_Step3b.BinomialModels_BestModel.Females.RData"));
}
...
if (FALSE){
  ...
  ks=c(10,8);
  k1 = ks[1]; k2 = ks[2];
  frmla  = p ~ ti(z, bs = "ts", k = k1) + ti(s, bs = "ts", k = k1) + ti(z, t, bs = "ts", k = k2) + 
                  s(s,h,bs="fs",k=k1);
  mdl_bestRE = mgcv::gam(frmla,family=famB,data=dfrDatpp,select=TRUE,method="REML",
                         drop.unused.levels=FALSE);
  wtsUtilities::saveObj(mdl_bestRE, file.path(dirThs,"rda_Step3b.BinomialModels_BestModelRE.Females.RData"));
