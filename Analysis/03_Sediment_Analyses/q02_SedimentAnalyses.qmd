...
#| cache: true
  #--plot map of phi
  prj  = stars::st_transform_proj(rasters[[2]],crs=bmls$map_scale$crs);
  p = ggplot() + bmls$land + bmls$bathym + bmls$theme + 
        stars::geom_stars(data=prj) + 
        bmls$map_scale + 
...
#| fig-cap: "Distributions of bottom temperature and interpolated mean grain size (phi units) and sorting with haul depth at NMFS SBS haul locations (2013-2018)."
#| dependson: SedAnal2_DoInterpolation
  ...
  if (!isPDF) print(p_vsd) else {
    lbl = getLabel();
    cap = paste0("Distributions of bottom temperature and interpolated mean grain size (phi units) and sorting with NMFS haul depth.");
    pth = file.path(dirFigs,basename(xfun::with_ext(knitr::fig_path(),def_ext)));
    lstFigs[[lbl]] = list(lbl=lbl,cap=cap,pth=pth,wid=def_wid,dpi=def_dpi);
...
#| fig-cap: "Pairs plot showing correlations among depth, bottom temperature, and interpolated mean grain size and sorting sediment characteristics at NMFS SBS haul locations (2013-2018)."
#| dependson: SedAnal2_DoInterpolation
  #--make pairs plots 
  p_col = GGally::ggpairs(dfrHDp)
  if (!isPDF) print(p_col) else {
    lbl = getLabel();
    cap = paste0("Pairs plot showing correlations among depth and bottom temperature at NMFS survey haul locations and interpolated mean grain size and sorting sediment characteristics.");
    pth = file.path(dirFigs,basename(xfun::with_ext(knitr::fig_path(),def_ext)));
    lstFigs[[lbl]] = list(lbl=lbl,cap=cap,pth=pth,wid=def_wid,dpi=def_dpi);
