...
#----function to predict values based on a model
  prdMod<-function(mdl,trms,lst,type="response",keep=NULL,p=0.05){
    ...
    #--calculate predictions on link scale to get correct lci/uci
    prd = dplyr::bind_cols(
              dfr,
              tibble::as_tibble(
                mgcv::predict.gam(mdl,dfr,type="link",
                                  terms=trms,se.fit=TRUE),
              ) |> 
    ...
    if (type=="response"){
      ...
      inv_link = gratia::inv_link(mdl);#--get inverse link function
      prd = prd |> dplyr::mutate(emp_sel=inv_link(emp_sel),
                                 lci=inv_link(lci),
                                 uci=inv_link(uci));
    }
    if (!is.null(keep)){
...
#| results: asis
#| fig-cap: "Estimated selectivity for arbitrary years, $S(z)$, from the model regarded as the most satisfactory for each sex (Model 3, in both cases)."
#| fig-width: 9
#| fig-asp: 0.6
  dfr = dplyr::bind_rows(
