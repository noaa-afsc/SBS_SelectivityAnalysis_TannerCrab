...
#--ln(r) = ti(z) + ti(d) + MISSING[ti(t)] + MISSING[ti(f))] + MISSING[ti(s)] + 
#--        ti(z,d) + MISSING[ti(z,t)] + ti(z,f) + MISSING[ti(z,s)] + 
#--        MISSING[ti(d,t)] + MISSING[ti(d,f)] + MISSING[ti(d,s)] + 
#--        MISSING[ti(t,f)] + MISSING[ti(t,s)] + 
#--        MISSING[ti(f,s)] +
#--        RE(h)
frmla  = p~ti(z,bs="ts") + ti(d,bs="ts") + 
           ti(z,d,bs="ts") + ti(z,f,bs="ts") + 
           s(h,bs="re");
mdl = gam(data=dfrDatRE,family=fam,formula=frmla,weights=n,select=TRUE,scale=0,offset=lnq,method="REML");
summary(mdl);
#plot(mdl);
...
wtsUtilities::saveObj(mdlsB,     paste0("selModels.BinomRE.All.", tolower(x_),"s.RData"));
wtsUtilities::saveObj(mdlsB[[3]],paste0("selModels.BinomRE.Best.",tolower(x_),"s.RData"));
