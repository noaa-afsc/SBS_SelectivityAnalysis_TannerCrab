...
device = "png";
pltctr = 1;
plotFN<-function(n,dev=device){paste0("plots4b_Females_",wtsUtilities::formatZeros(n),".",dev)}
...
for (iy in seq(1982,2019,intv)){
  ...
  p2 = ggplot2::ggplot() +
        ...
        #scale_colour_viridis_d(option="plasma",aesthetics=c("colour","fill")) +
        coord_cartesian(ylim=c(0,NA)) +
        labs(y="predicted catchability (ratio)",colour="year",fill="year") +
...
for (uG in uGs){
  ...
  p = ggplot2::ggplot() +
        ...
        ggplot2::geom_point(data=tmp,mapping=aes(x=z,y=mnR,shape=numI,colour=y),size=3,alpha=0.6) +
...
ggsave(paste0("plot4b_Females_AllAnnualCatchabilityCurves.",device),plot=p,width=8,height=5);
