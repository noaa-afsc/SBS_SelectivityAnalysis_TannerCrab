...
x = "female";
#--get best model
mdl = wtsUtilities::getObj(file.path(dirThs,"rda_GaussianModels_BestModel.RData"));


#--define grid for sizes
grd_z = seq(17.5,132.5,5);
...
dfrMnRp = dfrRbyHp |> 
              ...
              dplyr::summarize(mnR1=Hmisc::wtd.mean(prdR,weights=1/invwgt1,normwt=FALSE,na.rm=TRUE),
                               seR1=sqrt(Hmisc::wtd.var(prdR,weights=1/invwgt1,normwt=FALSE,na.rm=TRUE)),
                               ...
                               mnR2=Hmisc::wtd.mean(prdR,weights=1/invwgt2,normwt=FALSE,na.rm=TRUE),
                               seR2=sqrt(Hmisc::wtd.var(prdR,weights=1/invwgt2,normwt=FALSE,na.rm=TRUE)),
                               lower2=Hmisc::wtd.quantile(prdR,weights=1/invwgt2,normwt=FALSE,na.rm=TRUE,probs=0.10),
                               upper2=Hmisc::wtd.quantile(prdR,weights=1/invwgt2,normwt=FALSE,na.rm=TRUE,probs=0.90),
                               mnR3=Hmisc::wtd.mean(prdR,weights=1/invwgt3,normwt=FALSE,na.rm=TRUE),
                               seR3=sqrt(Hmisc::wtd.var(prdR,weights=1/invwgt3,normwt=FALSE,na.rm=TRUE)),
                               lower3=Hmisc::wtd.quantile(prdR,weights=1/invwgt3,normwt=FALSE,na.rm=TRUE,probs=0.10),
                               upper3=Hmisc::wtd.quantile(prdR,weights=1/invwgt3,normwt=FALSE,na.rm=TRUE,probs=0.90)
                              ) |> 
              dplyr::ungroup();
...
dfrMnRbyYp = dfrRbyHp |> dplyr::filter(!is.na(t)) |> #--trouble with missing temps in 1997 for some hauls
...
wtsUtilities::saveObj(dfrMnRbyYp,"rda_GaussianModels_BestEstSelsByYr.RData");
wtsUtilities::saveObj(dfrMnRp,"rda_GaussianModels_BestEstMnSels.RData");
