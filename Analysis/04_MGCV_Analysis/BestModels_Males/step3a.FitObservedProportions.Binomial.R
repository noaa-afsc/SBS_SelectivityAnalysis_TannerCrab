...
#--set up BINOMIAL models for lnR
...
if (FALSE){
  #--ln(r) = ti(z)
  k = 10;
  frmla = cbind(nN,nB)~s(z,bs="ts",k=k);
  mdlB_1DZa = estSelRatio(dfrDatp,formula=frmla,family=famB,offset=NULL);
  wtsUtilities::saveObj(mdlB_1DZa,file.path(dirThs,"rda_mdlB_1DZa.RData"))
}
if(!exists(mdlB_1DZa)) mdlB_1DZa = wtsUtilities::getObj(file.path(dirThs,"rda_mdlB_1DZa.RData"))
mdl = mdlB_1DZa$model;
ggplot(mdlB_1DZa$results,aes(x=z)) + 
...
IF (FALSE){
 ...
 #--         ti(d) + ti(t) + ti(f) + ti(s) +
  ...
  mdlB_ZE2D  = estSelRatio(dfrDatp,frmla,family=famB);
  wtsUtilities::saveObj(mdlB_ZE2D,file.path(dirThs,"rda_mdlB_ZE2D.RData"))
}
if(!exists(mdlB_ZE2D)) mdlB_ZE2D = wtsUtilities::getObj(file.path(dirThs,"rda_mdlB_ZE2D.RData"))
mdl = mdlB_ZE2D$model;
gam.check(mdl)
plot(mdl);
plot(mdl,residuals=TRUE)
...
AIC(mdlB_1DZa$model,mdlB_ZE2D$model)
...
IF (FALSE){
 ...
 #--         ------- + ti(t)  +  ti(f) + ---- +
  ...
  mdlB_ZE2Dsig  = estSelRatio(dfrDatp,frmla,family=famNrml);
  wtsUtilities::saveObj(mdlB_ZE2Dsig,file.path(dirThs,"rda_mdlB_ZE2Dsig.RData"))
}
if(!exists(mdlB_ZE2Dsig)) mdlB_ZE2Dsig = wtsUtilities::getObj(file.path(dirThs,"rda_mdlB_ZE2Dsig.RData"))
mdl = mdlB_ZE2Dsig$model;
summary(mdl)
gam.check(mdl)
plot(mdl);
plot(mdl,residuals=TRUE)
...
AIC(mdlB_1DZa$model,mdlB_ZE2D$model,mdlB_ZE2Dsig$model)
...
IF (FALSE){
  ...
  frmla  = p~ti(z,bs="ts",k=k)   + 
             ...
             ti(f,s,bs="ts",k=k2);
  mdlB_All2D  = estSelRatio(dfrDatp,frmla,family=famNrml);
  wtsUtilities::saveObj(mdlB_All2D,file.path(dirThs,"rda_mdlB_All2D.RData"))
}
if(!exists(mdlB_All2D)) mdlB_All2D = wtsUtilities::getObj(file.path(dirThs,"rda_mdlB_All2D.RData"))
mdl = mdlB_All2D$model;
summary(mdl)
gam.check(mdl)
plot(mdl);
plot(mdl,residuals=TRUE)
...
DHARMa::plotQQunif(simResids);
DHARMa::plotResiduals(simResids);
AIC(mdlB_1DZa$model,mdlB_ZE2D$model,mdlB_ZE2Dsig$model,mdlB_All2D$model)
...
IF (FALSE){
  ...
  frmla  = p~ti(z,bs="ts",k=k)   + 
             ...
             ti(t,f,bs="ts",k=k2) + 
             ti(f,s,bs="ts",k=k2);
  mdlB_All2Dsig  = estSelRatio(dfrDatp,frmla,family=famNrml);
  wtsUtilities::saveObj(mdlB_All2Dsig,file.path(dirThs,"rda_mdlB_All2Dsig.RData"))
}
if(!exists(mdlB_All2Dsig)) mdlB_All2Dsig = wtsUtilities::getObj(file.path(dirThs,"rda_mdlB_All2Dsig.RData"))
mdl = mdlB_All2Dsig$model;
summary(mdl);
gam.check(mdl)
plot(mdl);
plot(mdl,residuals=TRUE)
...
DHARMa::plotResiduals(simResids);
AIC(mdlB_1DZa$model,
    mdlB_ZE2D$model,mdlB_ZE2Dsig$model,
    mdlB_All2D$model,mdlB_All2Dsig$model)
...
IF (FALSE){
  ...
  mdlB_All3D  = estSelRatio(dfrDatp,frmla,family=famNrml);
}
